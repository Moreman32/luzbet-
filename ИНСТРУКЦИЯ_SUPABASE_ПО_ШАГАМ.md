...
- сохранение прогноза.
- загрузка и сохранение казино;
- загрузка и синхронизация ачивок;
- загрузка и сохранение плей-офф.
...
Пользовательская запись после этого уже должна идти через Functions.

Но это ещё не полный финал по безопасности.

Останутся:

- публичные чтения для лидерборда;
- админка `admin.html`;
- финальная настройка RLS и policies.
