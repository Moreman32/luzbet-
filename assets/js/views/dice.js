...
export async function mount(root,{app}){
 ...
 const play=actionButton("ПЕРЕДАТЬ ДЕЛО В ОТДЕЛ ВЕРОЯТНОСТЕЙ",async()=>{
  ...
  app.setBalance(r.round.balance??app.me.balance); await app.refreshMe(); won?sfx.cash():sfx.error(); const mctx=won&&chance<=15?"dice.longshotWin":!won&&chance>=80?"dice.safeLoss":won?"dice.win":"dice.loss"; toast(sessionMeme({won,lost:!won,balance:r.round.balance??app.me.balance,award:won&&chance<=15?"diceLong":!won&&chance>=80?"diceSafeLoss":null})||meme(mctx),won?"ok":"error");
