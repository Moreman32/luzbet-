...
export async function mount(root, { app }) {
  ...
  const tiles = [
    ...
    ["mines", "Mines", "Депозитные ячейки. Некоторые обслуживаются отделом внезапных проверок.", () => h("div",{class:"mines-art"},"◆  ◆  💣  ◆"), "скоро"],
    ["higher_lower", "Больше / Меньше", "52 уникальные карты. Равный номинал исключён, а коэффициенты считает карточная бухгалтерия.", () => h("div",{class:"hl-art"},"A♠  ?  K♥"), "97% RTP"],
  ];
  ...
  root.append(h("div", { class: "container stack" },
    h("section", { class: "hero" },
      h("div", { class: "card gilded stack" },
        h("div", { class: "eyebrow" }, "Добрый вечер, " + (me.displayName || me.username) + ". Касса уже нервничает."),
        h("h1", { class: "hero-title" }, "Казино, которое ", h("em", {}, "не подкручивает"), "."),
        h("p", { class: "muted", style: { maxWidth: "520px" } }, "Каждый исход фиксируется хешем до ставки и проверяется после. Смеяться над финансовыми решениями — можем. Подкручивать — нет. У нас и без этого всё неплохо."),
