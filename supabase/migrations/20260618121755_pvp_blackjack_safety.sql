alter table public.casino_pvp_blackjack_rooms
  add column if not exists settled_at timestamptz null,
  add column if not exists cancel_refunded_at timestamptz null;

create or replace function public.apply_pvp_blackjack_finish(
  p_room_id text,
  p_host_code text,
  p_host_name text,
  p_guest_code text,
...
create or replace function public.apply_pvp_blackjack_cancel_refund(
  p_room_id text,
  p_host_code text,
  p_host_name text,
  p_bet integer
...
declare
  ...
  v_existing public.casino_pvp_blackjack_rooms%rowtype;
...
begin
  ...
  select *
   ...
   where room_id <> p_room_id
     ...
     and (host_code = p_guest_code or guest_code = p_guest_code)
   order by updated_at desc
   limit 1
   for update;

  if found then
    raise exception 'you already have active room';
  end if;
