...
function isIgnoredEvent(ev: any) {
  const game = String(ev?.game || "");
  const source = String(ev?.meta?.source || "");
  const status = String(ev?.meta?.status || "");

  if (game === "system" && source === "balance_reconciliation") {
    return true;
  }

  if (ev?.event_type === "round" && status === "started") {
    return true;
  }
  ...
  try {
    ...
    while (true) {
      const { data, error } = await supabase
        .from("casino_events")
        .select("game,event_type,bet,payout,delta,created_at,meta")
    ...
    const filteredEvents = events.filter((ev) => !isIgnoredEvent(ev));
    ...
    for (const ev of filteredEvents) {
      ...
      const game = byGame[gameKey];

      const bet = Number(ev.bet || 0);
      const payout = Number(ev.payout || 0);
      const delta = Number(ev.delta || 0);
      const countsAsRound = ev.event_type === "round";

      for (const bucket of [summary, day, game]) {
        bucket.bet_sum += bet;
