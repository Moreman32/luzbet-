...
Deno.serve(async (req) => {
  ...
  try {
    ...
    let query = sb.from("casino_pvp_chicken_rooms").select("*").or(`host_code.eq.${code},guest_code.eq.${code}`);
    ...
    if (!row) return json({ ok: false, error: "pvp chicken room not found" }, 404);

    const room = hydratePvpChickenRoom(row as Record<string, unknown>);
    ...
    return json({ ok: false, error: e instanceof Error ? e.message : "get-pvp-chicken-room failed" }, 500);
  }
});
