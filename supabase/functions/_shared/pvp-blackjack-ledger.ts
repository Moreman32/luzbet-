import type { PvpBlackjackRoom } from "./pvp-blackjack.ts";
...
export async function ensurePvpBlackjackRoomLedger(sb: any, room: PvpBlackjackRoom) {
  ...
  if (room.status === "finished" && !room.settled_at) {
    const resolution = room.resolution && typeof room.resolution === "object"
      ...
      : {};
    const { data, error } = await sb.rpc("apply_pvp_blackjack_finish", {
  ...
  if (room.status === "cancelled" && !room.cancel_refunded_at) {
    const { data, error } = await sb.rpc("apply_pvp_blackjack_cancel_refund", {
      p_room_id: room.room_id,
      p_host_code: room.host_code,
      p_host_name: room.host_name || "",
