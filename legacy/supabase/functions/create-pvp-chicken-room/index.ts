...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const [{ data: participant, error: participantError }, { data: casinoRow, error: casinoError }, { data: existingRoom, error: roomError }] = await Promise.all([
      ...
      sb.from("casino_pvp_chicken_rooms").select("room_id, status").eq("host_code", code).in("status", ["waiting", "active"]).order("updated_at", { ascending: false }).limit(1).maybeSingle(),
    ]);
    ...
    return json({ ok: false, error: e instanceof Error ? e.message : "create-pvp-chicken-room failed" }, 500);
  }
});
